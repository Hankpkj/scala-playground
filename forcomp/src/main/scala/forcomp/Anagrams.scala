...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val raw = w.toLowerCase().toCharArray().toList.groupBy(t => t).map((x, xs) => (x, xs.length))
    (SortedMap[Char, Int]() ++ raw).toList
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsHelper(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) {
        ...
        for (
          i <- combination if dictionaryByOccurrences.keySet(i);
          j <- dictionaryByOccurrences(i);
          k <- sentenceAnagramsHelper(subtract(o, i))
        ) yield { j :: k }
      }
    }
