...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList = {
    isEmpty match {
      case true => Nil
      case false => new Cons(mostRetweeted,remove(mostRetweeted).descendingByRetweet)
    } 
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    ...
    (left.isEmpty, right.isEmpty) match {
      case (true, true) => elem
      case (true, false) => max(right.mostRetweeted,elem) 
      case (false, true) => max(left.mostRetweeted,elem)
      case (false, false) => max(left.mostRetweeted,max(left.mostRetweeted,elem))
    }
...
object GoogleVsApple:
  ...
  def f(l: List[String]) : TweetSet = TweetReader.allTweets.filter(t => l.exists(s => t.text.contains(s)))

  lazy val googleTweets: TweetSet = f(google)
  lazy val appleTweets: TweetSet = f(apple)
