...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    (chars.foldLeft(Map[Char,Int]()) (incr(_,_))).iterator.toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val sorted = freqs.sortWith((f1, f2) => f1._2 < f2._2)
    def toLeaf(c: Char, w: Int):Leaf = Leaf(c, w)
    sorted.map(toLeaf)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: ts => {
      val newCodeTree = makeCodeTree(left, right) :: ts
      newCodeTree.sortWith((t1, t2) => weight(t1) < weight(t2))
    }
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def sequence(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) => c :: sequence(tree, bits)
      case Fork(l, _, _, _) if bits.head == 0 => sequence(l, bits.tail)
      case Fork(_, r, _, _) => sequence(r, bits.tail)
    }
